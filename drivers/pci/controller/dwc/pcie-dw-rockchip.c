...
//wangwenjing begin
static int pcie_mode = 0;
static int __init pcie_mode_check(char *options)
{
	if (!options || !*options)
		return 1;
	
	if (strncmp(options, "pcie", 4) == 0) {
		pcie_mode = 1;
	}else
		pcie_mode = 0;
	return 1;
}
__setup("pcie_mode=", pcie_mode_check);
...
{
	//wangwenjing add begin
	struct gpio_desc *pedet_gpio = NULL;
	pedet_gpio = devm_gpiod_get_optional(&pdev->dev, "pedet", GPIOD_IN);
	if(NULL != pedet_gpio){
		if (IS_ERR(pedet_gpio)) {
	        dev_info(&pdev->dev, "invalid pedet-gpios property in node\n");
		}else{
			if(0 == gpiod_get_value(pedet_gpio)){
				dev_info(&pdev->dev, "pedet_gpio is low for sata mode\n");
				gpiod_put(pedet_gpio);
				return -ENODEV;
			}
			gpiod_put(pedet_gpio);
		}
